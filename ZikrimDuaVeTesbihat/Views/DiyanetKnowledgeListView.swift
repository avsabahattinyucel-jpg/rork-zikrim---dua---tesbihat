...
struct DiyanetKnowledgeListView: View {
    ...
    init(store: DiyanetKnowledgeStore, initialType: DiyanetContentType? = nil) {
        self.store = store
    ...
    private var filteredRecords: [DiyanetKnowledgeRecord] {
        ...
        return store.records.filter { record in
            let typeMatches = selectedType == nil || record.type == selectedType
