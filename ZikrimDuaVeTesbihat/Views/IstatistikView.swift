...
struct IstatistikView: View {
    ...
    var body: some View {
        ...
        .navigationTitle(L10n.string(.istatistik))
    ...
    private var lockedContent: some View {
        ScrollView {
            VStack(spacing: 18) {
                ...
                VStack(spacing: 8) {
                    Text(L10n.string(.detayliIstatistik))
                        .font(.title3.bold())
        ...
        VStack(spacing: 16) {
            HStack(alignment: .top) {
                VStack(alignment: .leading, spacing: 4) {
                    Label(.bugun, systemImage: "sun.max.fill")
                        .font(.caption.bold())
                ...
                VStack(alignment: .trailing, spacing: 8) {
                    ...
                    Text(L10n.format(.numberFormat, Int64(storage.profile.currentStreak)))
            ...
            VStack(alignment: .leading, spacing: 8) {
                HStack {
                    Text(.gunlukHedef)
                        ...
                        .foregroundStyle(.white.opacity(0.7))
                    Spacer()
                    Text(L10n.format(.dailyGoalProgressFormat, storage.profile.dailyGoal, Int(progress * 100)))
                        .font(.caption.bold())
                        .foregroundStyle(.white)
                }
    ...
    private var lifetimeStatsRow: some View {
        HStack(spacing: 12) {
            ...
            GlowStatCard(
                title: L10n.string(.istatistikEnUzunSeri),
                value: L10n.format(.daysCountFormat, Int64(storage.profile.longestStreak)),
                icon: "trophy.fill",
        ...
        VStack(alignment: .leading, spacing: 14) {
            HStack {
                Text(.haftalikOzet)
                    .font(.headline)
                Spacer()
                Text(L10n.format(.totalCountFormat, Int64(weeklyData.reduce(0) { $0 + $1.totalCount })))
                    .font(.caption.bold())
                    .foregroundStyle(.secondary)
            }
            ...
            ZStack {
                if index < 3 {
                    ...
                    Circle()
                        ...
                        .frame(width: 32, height: 32)
                    Text(L10n.format(.indexNumberFormat, Int64(index + 1)))
